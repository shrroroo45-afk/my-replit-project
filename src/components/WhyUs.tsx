...
export default function WhyUs() {
  const t = useT();
  const points = [
    { icon: <Headphones size={22} />, title: t('Professional Support', 'دعم احترافي'), desc: t('Dedicated support from start to finish via phone and WhatsApp.', 'دعم متواصل من البداية للنهاية عبر الهاتف والواتساب.') },
    { icon: <MapPin size={22} />, title: t('Real-Time Tracking', 'تتبع مباشر'), desc: t('Monitor your shipment live at every stage of the journey.', 'تابع شحنتك لحظة بلحظة في كل مرحلة.') },
    { icon: <FileCheck size={22} />, title: t('Customs Clearance', 'تخليص جمركي'), desc: t('We handle all paperwork and customs procedures seamlessly.', 'نتولى جميع الإجراءات الجمركية بسلاسة تامة.') },
    { icon: <DollarSign size={22} />, title: t('Competitive Pricing', 'أسعار تنافسية'), desc: t('Transparent rates with no hidden fees. Best value guaranteed.', 'أسعار شفافة بدون رسوم مخفية. أفضل قيمة مضمونة.') },
    { icon: <Settings size={22} />, title: t('Custom Solutions', 'حلول مخصصة'), desc: t('Tailored logistics plans designed for your specific business needs.', 'خطط لوجستية مصممة خصيصاً لاحتياجات عملك.') },
    { icon: <Shield size={22} />, title: t('Secure & Insured', 'أمان وتأمين'), desc: t('Every shipment is fully insured and carefully handled.', 'كل شحنة مؤمنة بالكامل ويتم التعامل معها بعناية.') },
  ];
